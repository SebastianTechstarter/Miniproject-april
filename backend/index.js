  ...
  try {
    ...
    if (!title && !author && !pages && !publisher && !year && !category) {
      return res.status(400).json({
        message: "Bitte mindestens ein Suchkriterium angeben! Danke :-)",
      });
    }
    ...
    books.filter((book) => {
      if (book.category === category) {
        matchesFields.push(book);
      }
    });
    if (books.length === 0) {
      return res.status(404).json({ message: "Keine Bücher gefunden" });
    }
    res.status(200).json(matchesFields);
  } catch (err) {
...
app.delete("/books", (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      deletedCount: books.length - filteredBooks.length,
    });
  } catch (err) {
    res
      .status(500)
